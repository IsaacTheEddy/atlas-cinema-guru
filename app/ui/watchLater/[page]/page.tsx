import { fetchFavorites, favoriteExists, fetchWatchLaters } from "@/lib/data";
...
export default async function Page({ params }: { params: { page?: string } }) {
  ...
  const titleList = await fetchWatchLaters(currentPage, email);

  const nextPageTitles = await fetchWatchLaters(currentPage + 1, email);
